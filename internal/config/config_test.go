...
func TestLoad_IdempotencyRouteEnvironmentOverrides(t *testing.T) {
	...
	t.Setenv("APP_IDEMPOTENCY_HTTP_PATHS", "[/api/v1/notifications/send, /api/v1/notifications/providers/receipt]")
	t.Setenv("APP_IDEMPOTENCY_GRPC_METHODS", "[/platform.notification.v1.NotificationService/Send]")
	cfg, err := Load(path)
	if err != nil {
		t.Fatal(err)
	}
	if got := strings.Join(cfg.Idempotency.HTTPPaths, ","); got != "/api/v1/notifications/send,/api/v1/notifications/providers/receipt" {
		t.Fatalf("HTTPPaths = %q", got)
	}
	if got := strings.Join(cfg.Idempotency.GRPCMethods, ","); got != "/platform.notification.v1.NotificationService/Send" {
